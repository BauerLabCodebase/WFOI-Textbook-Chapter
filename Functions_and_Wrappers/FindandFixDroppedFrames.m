...
fixed_by_channel=reshape(data,OGsize(1),OGsize(2),runInfo.numCh,[]);
    ...
    ignore_frames=[runInfo.darkFramesInd(end):runInfo.darkFramesInd(end)+2]; %ignore the frames that
    ...
    for i=1:runInfo.numCh
        ...
        jumps(i)={ find(  sqrt((gradient(squeeze(mu_reshape(i,:))).^2)) >  .2*sqrt(mean(gradient(mu_reshape(i,:)).^2)))};
    ...
    DarkFrame_data=DarkFrame_data(:,new_order,:); %reorder the channels. If dropped frame in channel 4 then channel 4 becomes channel 1.
    ...
    if size(NonDarkFrame_data,2)~=runInfo.totNum_frame
